...
function shop(u){
  ...
  if(items[no].ct==0){
      ...
      <div class="cartproduct" id="${items[u].id}">
       <div>
        <img src="${items[u].img1}" alt="Product Image"  class="cartproductimg">
          ...
          <div class="carttrash">
              <i class="fa-solid fa-trash trash" style="color: #bb2b07;" id="removetotrash" onclick="removeitem(${items[u].id})"></i>
          </div>
          <p></p>
      </div>`;
   ...
   var selecteditem=items[u].id
   let findmatch=basket.find((x)=>x.id===selecteditem)
  if(findmatch===undefined){
    ...
    findmatch.item+=1
    findmatch=basket.find((x)=>x.id===selecteditem)
   // console.log(findmatch.item)
    document.getElementById(items[u].id1).innerHTML=findmatch.item
...
const calculate=()=>{
  let sum=0;
  let res=basket.map((x)=>{
    sum+=Number(x.item)
  })
  let amt=sum*14
  dollarprice.innerHTML=amt+".00"
  console.log(sum*14)
